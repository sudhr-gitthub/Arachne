...
export default function Home() {
  ...
  const isDatabaseActive = activeSidebarTab === "Database";
  const isSettingsActive = activeSidebarTab === "Settings";
  const isLogisticsActive = activeHeaderTab === "Logistics" && !isDatabaseActive && !isSettingsActive;
  const isSurveillanceActive = activeHeaderTab === "Surveillance" && !isDatabaseActive && !isSettingsActive;
  
  const isDashboardActive = activeSidebarTab === "Dashboard" && !isLogisticsActive && !isSurveillanceActive && !isDatabaseActive && !isSettingsActive;
  const isCommandMapActive = activeSidebarTab === "Map" && !isLogisticsActive && !isSurveillanceActive && !isDatabaseActive && !isSettingsActive;
  const isNetworkActive = activeSidebarTab === "Network" && !isLogisticsActive && !isSurveillanceActive && !isDatabaseActive && !isSettingsActive;
  const isAnalyticsActive = activeSidebarTab === "Analytics" && !isLogisticsActive && !isSurveillanceActive && !isDatabaseActive && !isSettingsActive;
  const isPredictionsActive = activeSidebarTab === "Predictions" && !isLogisticsActive && !isSurveillanceActive && !isDatabaseActive && !isSettingsActive;
  const isReportsActive = activeSidebarTab === "Reports" && !isLogisticsActive && !isSurveillanceActive && !isDatabaseActive && !isSettingsActive;
  ...
  return (
    <div className="flex flex-col gap-6 w-full">
      ...
      {isDashboardActive && <DashboardView />}
      {isNetworkActive && (
        <div className="flex flex-col min-h-[650px] w-full">
          <NexusContainer />
        </div>
      )}
      {isAnalyticsActive && <AnalyticsView />}
      {isPredictionsActive && <PredictionsView />}
      {isReportsActive && <ReportsView />}
      {isDatabaseActive && <DatabaseView />}
      {isSettingsActive && <SettingsView />}

      {/* Fallback Initializing View when not in any known views */}
      {!isCommandMapActive && !isNetworkActive && !isLogisticsActive && !isSurveillanceActive && !isDatabaseActive && !isSettingsActive && !isDashboardActive && !isAnalyticsActive && !isPredictionsActive && !isReportsActive && (
